...
class LocalVideoDataset(Dataset):
    def __init__(
        ...
        with open(meta_path, 'r') as fin:
            for line in fin:
                ...
                folder_prefix,sub = folder.split("_")[0], folder.split("_")[1]
                self.keys.append(folder_prefix)
    ...
    def __getitem__(self, index):
        try:
            example = dict()
            folder_prefix = self.keys[index]
            suffix = random.choice(self.sub_dict[folder_prefix])
            clip_name = f"{folder_prefix}_{suffix}"
            frame_num = self.frame_dict[clip_name]
            start_frame_idx = random.randint(0, frame_num - self.num_frame)
