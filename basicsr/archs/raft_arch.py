...
class CorrBlock:
    ...
    def __call__(self, coords):
        ...
        # float 16
        return out.permute(0, 3, 1, 2).contiguous().float().to(torch.float16)
...
def forward_interpolate(flow):
    ...
    return torch.from_numpy(flow).float().to(torch.float16)
...
def bilinear_sampler(img, coords, mode='bilinear', mask=False):
    ...
    # 半精度
    grid = grid.float().to(torch.float16)
    img = F.grid_sample(img, grid, align_corners=True)

    if mask:
        mask = (xgrid > -1) & (ygrid > -1) & (xgrid < 1) & (ygrid < 1)
        return img, mask.float().to(torch.float16)
...
def coords_grid(batch, ht, wd):
    ...
    coords = torch.stack(coords[::-1], dim=0).float().to(torch.float16)
...
class RAFT(nn.Module):
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, upsample=True, test_mode=False):
        ...
        fmap1 = fmap1.float().to(torch.float16)
        fmap2 = fmap2.float().to(torch.float16)
