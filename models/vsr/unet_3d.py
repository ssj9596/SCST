...
from .resnet import InflatedConv3d, InflatedGroupNorm, get_sft
...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        # import pdb
        # pdb.set_trace()
        if use_sft and not sft_decoder_only:
            self.sft_first = get_sft(
                condition_in_channels=block_out_channels[0],
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            down_block = get_down_block(
                ...
                use_sft=use_sft and not sft_decoder_only,
                sft_type=sft_type
            )
        ...
        if mid_block_type == "UNetMidBlock3DCrossAttn":
            self.mid_block = UNetMidBlock3DCrossAttn(
                ...
                motion_module_kwargs=motion_module_kwargs,
                name=mid_name,
                use_sft=use_sft and sft_mid_block,
                sft_type=sft_type
            )
        else:
    ...
    def forward(
        ...
        # down
        residuals_copy = down_block_additional_residuals.copy()
        controlnet_condition_up = residuals_copy[0]
        residuals_copy = residuals_copy[1:]

        if self.sft_first:
            sample = self.sft_first(sample, controlnet_condition_up)
            
        down_block_res_samples = (sample,)
        for i, downsample_block in enumerate(self.down_blocks):

            controlnet_condition_up = residuals_copy[:len(downsample_block.resnets)]
            residuals_copy = residuals_copy[len(downsample_block.resnets)+1:]
            #  最后不需要更新residuals_copy
            if (
                ...
                sample, res_samples = downsample_block(
                    ...
                    mode=self.mode,
                    controlnet_conditions=controlnet_condition_up,
                )
            else:
                sample, res_samples = downsample_block(
                    hidden_states=sample,
        ...
        # mid controlnet_condition
        controlnet_condition_mid = mid_block_additional_residual
        ...
        sample = self.mid_block(
            ...
            self_attention_additional_feats=self_attention_additional_feats,
            mode=self.mode,
            controlnet_conditions=controlnet_condition_mid,
        )
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            # pixel condition from controlnet
            controlnet_condition_down = down_block_additional_residuals[-len(upsample_block.resnets) :]
            ...
            if (
                ...
                sample = upsample_block(
                    ...
                    self_attention_additional_feats=self_attention_additional_feats,
                    mode=self.mode,
                    controlnet_conditions=controlnet_condition_down,
    ...
    def from_pretrained_2d(
        ...
        print("==========================loading state_dict Start==================================")
        ...
        if len(u):
            ...
            print(u)
            raise ValueError("There should not Have unexpected keys!!!!")
